-- Otimizações para o sistema de atendimento
...
-- 4. Função para assumir conversa de forma atômica (evita conflitos)
...
AS $$
DECLARE
  conversa_assumida boolean := false;
BEGIN
  ...
  GET DIAGNOSTICS conversa_assumida = FOUND;
  RETURN conversa_assumida;
END;
$$;
...
DO $$
BEGIN
  -- Verificar se a publicação existe e adicionar as tabelas
  IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
    PERFORM pg_catalog.pg_publication_add_table('supabase_realtime', 'public.conversas');
    PERFORM pg_catalog.pg_publication_add_table('supabase_realtime', 'public.mensagens');
    PERFORM pg_catalog.pg_publication_add_table('supabase_realtime', 'public.contatos');
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- Ignorar erros se as tabelas já estiverem na publicação
  NULL;
END $$;
...
-- 8. Função para finalizar atendimento
...
AS $$
DECLARE
  atendimento_finalizado boolean := false;
BEGIN
  ...
  GET DIAGNOSTICS atendimento_finalizado = FOUND;
